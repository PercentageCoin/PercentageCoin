...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "percentagecoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"131.72.137.93", "131.72.137.93"}, //First official node
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. percentagecoin is probably already running."), addrBind.ToString().c_str());
        else
