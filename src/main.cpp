...
const string strMessageMagic = "percentagecoin Signed Message:\n";
...
int64 nMinimumInputValue = DUST_HARD_PCCIT;
...
{
    // percentagecoin: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    // percentagecoin
    // To limit dust spam, add nBaseFee for each output less than DUST_SOFT_PCCIT
    BOOST_FOREACH(const CTxOut& txout, vout)
        if (txout.nValue < DUST_SOFT_PCCIT)
            nMinFee += nBaseFee;
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 35 days
    nSubsidy >>= (nHeight / 50000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 10; // Retarget every 10 blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // percentagecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, percentagecoin - DarkGravity, written by Evan Duffield - evan@percentagecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, percentagecoin - DarkGravity v2, written by Evan Duffield - evan@percentagecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // percentagecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // percentagecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // percentagecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("percentagecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("percentagecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static percentagecoinMiner(CWallet *pwallet)
{
    printf("percentagecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("percentagecoin-miner");
    ...
    try { loop {
        ...
        printf("Running percentagecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("percentagecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&percentagecoinMiner, pwallet));
}
